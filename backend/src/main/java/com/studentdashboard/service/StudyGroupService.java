...
public class StudyGroupService {
    ...
    public StudyGroupDTO updateGroup(Long id, StudyGroupDTO groupDTO) {
        StudyGroup existingGroup = studyGroupRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Study group not found with id: " + id));
        
        // Verify that the user is the creator
        if (!existingGroup.getCreatorEmail().equals(groupDTO.getCreatorEmail())) {
            throw new RuntimeException("Only the group creator can update this group");
        }
        
        // Update fields
        existingGroup.setCreatorName(groupDTO.getCreatorName());
        existingGroup.setCreatorEmail(groupDTO.getCreatorEmail());
        existingGroup.setCreatorPhone(groupDTO.getCreatorPhone());
        existingGroup.setYear(groupDTO.getYear());
        existingGroup.setSemester(groupDTO.getSemester());
        existingGroup.setSpecialization(groupDTO.getSpecialization());
        existingGroup.setModule(groupDTO.getModule());
        existingGroup.setTopic(groupDTO.getTopic());
        
        // Validate max capacity is not less than current member count
        if (groupDTO.getMaxMembers() < existingGroup.getCurrentMembers()) {
            throw new RuntimeException("Max capacity cannot be less than current member count (" + existingGroup.getCurrentMembers() + ")");
        }
